...
class Author < ActiveRecord::Base
  ...
  def self.most_prolific_writer
    self.find_by_sql("select auth.name, count(art.id) as size from articles as art, authors as auth where art.author_id = auth.id order by size desc limit 1").first
  end
